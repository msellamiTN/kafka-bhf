    ...
    public class KafkaController : ControllerBase
    {
            ...
            [FromQuery] string? topic = null, [FromQuery] string? sendMode = null, [FromQuery] string? key = null)
        {
            try
            {
                ...
                // Envoi du message
                var result = await _producerService.SendMessageAsync(topic, key, message, isIdempotent, isAsync);
        ...
        public IActionResult GetStatus()
        {
            return Ok(new
            {
                ...
                supportedModes = new[] { "plain", "idempotent" },
                supportedSendModes = new[] { "sync", "async" }
            });
        }
    }
}
